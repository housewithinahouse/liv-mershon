...
1)Configuration:
	a)vars.php
		 ...
		 It's also where you find the superclass for everything: BasicWebElement.
		 vars.php is called on every script.
	b)js & css
		 These should live in the root folder. ie: /js/ and /css/
2)Building pages:
	a)BasicWebElement
	...
	b)Head
		...
		-add css, by calling addStyleSheet($name), the path is "/css/$name"
		-add less, by calling addLess($name), the path is "/css/$name"
		-add scripts, by calling addScript($name), the path is "/js/$name"
